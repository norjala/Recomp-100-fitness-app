...
const configSchema = z.object({
  ...
  // Competition
  COMPETITION_START_DATE: z.string().datetime().default('2025-08-04T00:00:00.000Z'),
  COMPETITION_END_DATE: z.string().datetime().default('2025-11-14T23:59:59.999Z'),
...
function validateConfig(config: AppConfig) {
  ...
  if (startDate >= endDate) {
    throw new Error('COMPETITION_END_DATE must be after COMPETITION_START_DATE');
  }
  
  // Validate admin usernames
  const adminUsernames = config.ADMIN_USERNAMES.split(',').map(u => u.trim()).filter(u => u);
  if (adminUsernames.length === 0) {
    throw new Error('At least one admin username must be specified in ADMIN_USERNAMES');
  }
  ...
  if (config.NODE_ENV === 'production') {
    ...
    if (config.DATABASE_URL.startsWith('./') && !config.DATABASE_URL.startsWith('./data/')) {
      console.warn('⚠️  WARNING: Consider using absolute path for DATABASE_URL in production');
    }
  }
  ...
  for (const type of allowedTypes) {
    if (!validMimeTypes.includes(type)) {
      console.warn(`⚠️  WARNING: Potentially invalid MIME type in ALLOWED_FILE_TYPES: ${type}`);
    }
  }
}
...
function logConfiguration(config: AppConfig) {
  ...
  console.log(`   Admin Users: ${config.ADMIN_USERNAMES.split(',').length} configured`);
  console.log(`   Competition: ${config.COMPETITION_START_DATE} to ${config.COMPETITION_END_DATE}`);
...
export function getAdminUsernames(): string[] {
  return getConfig().ADMIN_USERNAMES.split(',').map(u => u.trim()).filter(u => u);
}
...
export function getCorsOrigins(): string[] {
  return getConfig().CORS_ORIGINS.split(',').map(origin => origin.trim()).filter(origin => origin);
}

export function getAllowedFileTypes(): string[] {
  return getConfig().ALLOWED_FILE_TYPES.split(',').map(type => type.trim()).filter(type => type);
}